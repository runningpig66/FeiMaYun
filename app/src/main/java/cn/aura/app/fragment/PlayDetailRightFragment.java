...
import cn.aura.app.adapter.PlayDetailRightFragment_ListView_Adapter2;
import cn.aura.app.bean.PlayDetailBean;
...
public class PlayDetailRightFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new PlayDetailRightFragment_ListView_Adapter2(context, null, sid, uid, isBuy);
        fragment_playdeatil_right_listView1.setAdapter(adapter);
        parseJSON();
        return view;
    }

    public void setData(String catalogueString, String sid, String isBuy) {
    ...
    public void setSid(String sid, String isBuy, HashSet<String> sidSet) {
        this.sid = sid;
        this.isBuy = isBuy;
        sidLearned = sidSet;
        parseJSON();
            ...
            try {
                ...
                for (int i = 0; i < catalogueArray.length(); i++) {
                    ...
                    if (detailBean.getFtype().equals("video")) {//添加sid
                        ...
                        //遍历learned
                        for (String str : sidLearned) {
                            if (str.equals(detailBean.getSort())) {
                                detailBean.setLearn("1");
                                break;
                            }
                        }
                    }
                            ...
                            if (children1Object.has("children")) {
                                ...
                                for (int k = 0; k < children2Array.length(); k++) {
                                    ...
                                    if (children2Bean.getFtype().equals("video")) {
                                        ...
                                        for (String str : sidLearned) {
                                            if (str.equals(children2Bean.getSort())) {
                                                children2Bean.setLearn("1");
                                                break;
                                            }
                                        }
                                    }
...
//                printList(detailBeans);
                ...
                fragment_playdeatil_right_listView1.post(new Runnable() {
                    @Override
                    public void run() {
                        adapter.setData(detailBeans, sid, Util.getUid(), isBuy);
